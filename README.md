...
Ansible provisions and operates three Rocky Linux service VMs on Proxmox: a
fail-closed Mullvad download jail, a rootless media host that also carries the
access layer, and a rootless infrastructure host. Caddy and dnsmasq provide
split-horizon `*.fortwow.dev` service names with publicly trusted TLS; TrueNAS
supplies the NFS media and backup datasets. A nightly report-only security scan
measures patch, image, benchmark, exposure and credential posture without
changing any of it.
...
| `svc-infra` | Rootless self-hosted applications, Authelia SSO, Prometheus/Grafana, and the nightly verification and scan runners |
| TrueNAS | Manually managed NFS media and backup storage |

The inventory's `ansible_host` values are the canonical VM addresses.
`download_apps` describes the jailed applications, `infra_apps` and
`infra_secret_apps` the svc-infra catalog, and `caddy_services` the media and
infrastructure endpoints. See
...
| `make infra` | Converge `svc-infra` only |
...
- [Security](docs/security.md) — secrets, Proxmox trust, image policy, the VPN backstop, and the nightly scan
